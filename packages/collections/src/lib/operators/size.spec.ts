...
describe('size', () => {
  ...
  it('should emit the size', () => {
    testScheduler.run(({ cold, expectObservable }) => {
      ...
      const expected = 'a';
      expectObservable(source.pipe(size())).toBe(expected, {
        a: 3,
      });
    });
  });
  ...
  describe('observable functionality', () => {
    it('should not complete if the source never completes', () => {
      testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
        ...
        expectObservable(e1.pipe(size())).toBe(expected);
    ...
    it('should raise an error if the source raises an error', () => {
      testScheduler.run(({ hot, expectObservable, expectSubscriptions }) => {
        ...
        expectObservable(e1.pipe(size())).toBe(expected, { a: 3 });
        expectSubscriptions(e1.subscriptions).toBe(e1subs);
      });
    });
